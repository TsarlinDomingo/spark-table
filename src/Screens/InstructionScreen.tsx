...
type InstructionScreenProp = NativeStackScreenProps<RootStackParamList, 'Instructions'>

export default function InstructionScreen({ navigation }: InstructionScreenProp): JSX.Element {
  const questionId = Math.floor(Math.random() * 3);
  return (
    <View className="flex-1 flex-direction flex-col items-center justify-center bg-white space-y-8 px-8">
      ...
      <TouchableOpacity className="rounded-3xl bg-green-500 px-10 py-4 h-fit" onPress={() => navigation.navigate('Question', { questionId })}>
